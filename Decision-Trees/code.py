...
# Code starts here
le = LabelEncoder()
cat_df_train = X_train.select_dtypes(exclude=np.number)
cat_df_test = X_test.select_dtypes(exclude=np.number)

for x in list(cat_df_train):
    X_train[x] = le.fit_transform(X_train[x])
    X_test[x] = le.transform(X_test[x])

#X_train = pd.concat([X_train.select_dtypes(include=np.number),cat_df_train], 1)
#X_test = pd.concat([X_test.select_dtypes(include=np.number),cat_df_test], 1)
