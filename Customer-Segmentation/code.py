...
# Merge dataframes
transactions['n'] = 1
df = transactions.merge(offers, on='Offer #', how='left')
...
# visualize clusters
plt.scatter(x=matrix['x'], y=matrix['y'])
...
data = transactions.merge(clusters, on='Customer Last Name', how='left')

# merge `data` and `offers`
data = data.merge(offers, on='Offer #', how='left')
...
for x in data['cluster'].unique().tolist():
    ...
    new_df = data[data['cluster']==x]
    # sort cluster according to type of 'Varietal'
    counts = new_df['Varietal'].value_counts(ascending=False)
    # check if 'Champagne' is ordered mostly
    if counts.index[0] == 'Champagne':
        # add it to 'champagne'
        champagne[x] = counts[0]
